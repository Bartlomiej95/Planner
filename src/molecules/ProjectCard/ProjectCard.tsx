...
const ProjectCard = ({ titleProject, description, departments, projectId, projectTasks }: Props) => {
    
    // const percentProjectCompletion = ((amountFinsishedTasksInProject * 100)/(projectTasks.length)).toFixed(2);
    
    const getWeightOfTask = (task :Task) => {
    ...
    const countPercent = (tasks :Array<Task>) => {
        //funkcja przyjmuje tablicę zadań, a ma zwracać obiekt wyników wag zadań zrobionych, oraz sume wszystkich wag 
        let getPoints: number = 0; // jeśli zadanie jest skończone to dodaj jego wagę
        let allPoints: number = 0; // suma wag wszystkich zadań
    ...
    const countPercentProjectCompletion = (tasks :Array<Task>) => {
            ...
            <WrapperLabels>
                {
                    departments.map(item => (
                        <Label division={item} name="" type="card" getStatus={() => console.log('')} />
                    ))
                }
            </WrapperLabels>
